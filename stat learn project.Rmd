...
plot_usmap(data = vc_state, values = "ViolentCrimesMean", color = "black") +
   scale_fill_continuous(trans="sqrt", low = Rgb[3], high = Rgb[1],
                         name = "Viol Crimes 100K pop", limits=c(85,3049), label=scales::comma) +
  theme(legend.position = "right")
```

We can notice that the states with the highest average violent crimes rate are mostly located in the south-east. Moreover, the summary and the geographical plot suggest the presence of some outlier in the averages.
...
par(mfrow = c(1,2))
boxplot(violPerPop, outline = TRUE, col = Col[seq(1:4)], main = "Violent Crimes per 100K Pop")
boxplot(violPerPop, outline = FALSE, col = Col[seq(1:4)], main = "Violent Crimes per 100K Pop - Outliers not drawn")
par(mfrow=c(1,1))
```
...
library(plotly)
plot_ly(data=vc_cat_state, x = ~state, y = ~murdPerPop, type = 'bar', name = 'murders', marker = list(color = Rgb[1])) %>%
    add_trace(y = ~rapesPerPop, name = 'rapes', marker = list(color = Rgb[2])) %>%
    add_trace(y = ~robbbPerPop, name = 'robbery', marker = list(color = Rgb[3])) %>%
    add_trace(y = ~assaultPerPop, name = 'assault', marker = list(color = Rgb[4])) %>%
    layout(xaxis = list(title='State', tickangle = 270), yaxis=list(title = 'Count'), barmode = 'stack')
```
...
#nvc_state[nvc_state$NonViolentCrimesMean == max(nvc_state$ViolentCrimesMean),]
```
...
par(mfrow = c(1,2))
boxplot(nonViolPerPop, outline = TRUE, col = Col[seq(1:4)], main = "Non Violent Crimes Per Pop")
boxplot(nonViolPerPop, outline = FALSE, col = Col[seq(1:4)], main = "Non Violent Crimes Per Pop")
par(mfrow=c(1,1))
```
...
library(plotly)
plot_ly(data=nvc_cat_state, x = ~state, y = ~burglPerPop, type = 'bar', name = 'burglary',
        marker = list(color = Rgb[1])) %>%
    add_trace(y = ~larcPerPop, name = 'larceny', marker = list(color = Rgb[2])) %>%
    add_trace(y = ~autoTheftPerPop, name = 'auto theft', marker = list(color = Rgb[3])) %>%
    add_trace(y = ~arsonsPerPop, name = 'arson', marker = list(color = Rgb[4])) %>%
    layout(xaxis = list(title='State', tickangle = 270), yaxis=list(title = 'Count'), barmode = 'stack')
```
...
rm(list=setdiff(ls(), c("crimedata", "cleandf", "standf", "Col", Rgb)))
```
...
With a trial and error approach we came out with the best transformation: $\log(Y)+1$, of which the resulting density is shown the following figures[I].
...
Below, RIDGE and LASSO regression are implemented.
...
RIDGE regression decreases model complexity while keeping all variables in the model by shrinking the coefficients of low-variance components more than those of highvariance components. The more lambda increases, the more the coefficients are shrinkaged towards zero.
...
subset.data.frame(ridge.coef, abs(Coefficients)>0.1)
```

The predictors having an estimated coefficient greater than 0.1 are:

* the percentage of households with investment/rent income (pctWInvInc);
* the percentage of percentage of population who are divorced (TotalPctDiv);
* the percentage of African-American (racepctblack);
* the percentage of people living in areas classified as urban (pctUrban).
...
The most relevant predictors are again racepctblack, TotalPctDiv and pctWInvInc; however, as a result of choosing a higher lambda value, the associated estimated coefficients are smaller than before.
...
subset.data.frame(lasso.coef, abs(Coefficients)>0.1)
```

LASSO regression with $\lambda$.min selects 33 variables, collecting different kinds of information: ethnicity, age, income, education, marital status, houses occupation and many others.
Also in this case, the most relavant predictors (with estimated coefficient greater than 0.1) are almost the same as for RIDGE regression:

* the percentage of households with investment/rent income (pctWInvInc);
* the percentage of percentage of population who are divorced (TotalPctDiv);
* the percentage of African-American (racepctblack);
* the percentage of Hispanic-American (racePctHisp);
* the percentage of people living in areas classified as urban (pctUrban).
...
# number of predictors
p.se <- sum(abs(lasso.coef$Coefficients)>0)-1  #remove intercept

# coefficients different from zero 
subset.data.frame(lasso.coef, abs(Coefficients)>0)
```

LASSO regression with $\lambda$.1se results in a simpler model with 21 variables.
...
while the regression models reach barely sufficient results: even with a valuable adjusted $R^2$ of ???, the logarithmic
tranformation applied to the target variable led us to compute a MSLE of 0.???, which says that the average ratio between the
predicted value +1 and the true value +1 is ???. This is mainly due to the fact that a linear model (and in general a polinomial one,
which gave similar results), is not enough to fit the complexity of the data, as we expected given the complexity of the task even
from a social perspective. Furthermore, a huge limitation was the really poor quality of the LEMAS dataset,
the one gathered from the police departments, and this didn't allowed us to perform a parallel study to investigate a more local point of view.
...
but the modified expression $RSS+\lambda\sum_{j=1}^p\beta_j^2$ where $\lambda\geq0$ can be tuned in order
...
In KNN classification, the output is a class membership. An object is assigned to the class most common among its K nearest neighbors, where K is a positive integer. Note that the results is strongly local, because it just depends on the nearest K training instances to the target point.
