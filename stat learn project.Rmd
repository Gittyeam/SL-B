...
The probability to observe a value greater than 3 is 0.001349898. [ref biblio?]
...
X <- as.matrix(subset(corrdf, select = - c(ViolentCrimesPerPop)))
Y <- as.matrix(subset(corrdf, select =  c(ViolentCrimesPerPop)))
Y <- log(Y+1)
...
Y.train <- subset(Y, (row.names(Y) %in% train.sample))
Y.test <- subset(Y, (row.names(Y) %in% test.sample))
```
