...
summary(step.mod)
# trace=1
step.mod <- step(full.mod, steps=10,  trace=1, direction="backward")
```

```{r}
full <- full.mod
empty <- lm(Y.train~1, data=X.train, family=binomial)

step.mod <- step(empty, steps=30, trace=0, scope=list(lower=formula(empty), upper=formula(full)), direction="forward")
