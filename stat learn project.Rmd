...
vcorr <- cm[,dim(cm)[2]]
vnames <- names(vcorr[abs(vcorr)>0.5])

```

```{r}
pairs(cleandf[,colnames(cleandf)%in% vnames], pch = 20)
```

- osservazioni sulle correlate con violcrimes, e tra loro, tre macro aree, confounding , vedi pag wiki per storia crimine brutta :( 
- domande---

-  scelta response violent  


```{r , include=FALSE}
# Remove all variables from environment excpet the cleaned and the original dataset
rm(list=setdiff(ls(), c("cleandf","crimedata","standf", "Col")))
```
...
ME and ND northern states (almost canada)
...
According to the Tukey test, the states can be grouped in two main cluster:
...
ANCOVA .. different regressions for the two group
```{r}
