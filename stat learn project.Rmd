...
# linear model with training data
lm.fitPC <- lm(normnewdfcorr$ViolentCrimesPerPop~., data=pcdf, subset=tp)


# mean squared error on train
y.pred <- predict(lm.fitPC, newdata=pcdf[tp,])
MSE <- mean((normnewdfcorr$ViolentCrimesPerPop[tp]-y.pred)^2)
MSE 
...
# linear model with training data
lm.fitPC <- lm(log(normnewdfcorr$ViolentCrimesPerPop+1)~., data=pcdf, subset=tp)


# mean squared error on train
y.pred <- predict(lm.fitPC, newdata=pcdf[tp,])
MSE <- mean((log(normnewdfcorr$ViolentCrimesPerPop[tp]+1)-y.pred)^2)
MSE 
...
mm <- lm(data.matrix(normnewdfcorr[,48:65]) ~. , data=as.data.frame(pccorr$scores[,c(1,2,5,8,9,10)]))
