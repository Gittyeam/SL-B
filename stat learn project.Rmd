...
## Diagnostic Tests - Multiple linear regression: predictor = ViolentCrimesPerPop

Remove non predictive variables:
communityname, state, all crimes except variable ViolentCrimesPerPop
```{r}
coltodrop <- c(1,2, seq(103,120)[-17])
...
#### Linearity
The plots of residuals versus fitted values shows a little pattern, however it seems to indicate that there are linear associations in the data.
...
The RSE is 0.65 when the outlier is included in the regression and it is 0.59 when the outlier is removed. Since the RSE is used to compute all conﬁdence intervals and p-values, this can have implications for the interpretation of the fit. Moreover, dropping the outliers the $R^2$ increses from 0.67 to 0.7.
...
In the following few lines of code we perform a first manual skimming of stongly dependent variables removing, between the most correlated ones, the more redundant and meaningless.
...
polyformula <- eval(paste("normnewdfcorr$ViolentCrimesPerPop ~", paste(paste('poly(', names(as.data.frame(pccorr$scores[,c(1,2,5,8,9,10)])), ', 2)', sep=''), collapse=" + ")))
...
polyformula <- eval(paste("normnewdfcorr$ViolentCrimesPerPop ~", paste(paste('poly(', names(as.data.frame(pccorr$scores[,c(1,2,5,8,9,10)])), ', 3)', sep=''), collapse=" + ")))
...
m3 <- lm(data.matrix(normnewdfcorr[,81:98]) ~. , data=as.data.frame(pccorr$scores[,c(1,2,5,8,9,10)]))
