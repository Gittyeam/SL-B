...
compute all conﬁdence intervals and p-values, this can have implications for the interpretation of the fit. Moreover, dropping the
outliers the $R^2$ increses from 0.67 to 0.7.
Care should be taken in the decision of taking or dropping outliers.

#### High Leverage Points
In order to quantify an observation’s leverage, we compute the leverage statistic. If a given observation has a leverage statistic that
...
In the following few lines of code we perform a first manual skimming of stongly dependent variables removing, between the most
