...
```{r, include=FALSE}
# Remove all variables from environment excpet the cleaned and the original dataset
rm(list=setdiff(ls(), c("cleandf","crimedata","standf","corrdf")))
```
...
```{r}
regfit.full <- regsubsets(Y.train~., nvmax = 25, data= X.train)
...
best subset is limited when searching between large number of feautures (slow with 25 )... then use greedy approaches..
...
```{r}
step.mod <- step(full.mod, steps=20,  trace=0, direction="backward")
summary(step.mod)
```
...
step.mod <- step(empty.mod, steps=20,  trace=0, scope=list(lower=formula(empty.mod), upper=formula(full.mod)), direction="forward")
summary(step.mod)
```
...
??? ho definito X, Y e i relativi train e test in precedenza per la PCA come aveva fatto Nat, ma non come matrici (sotto non ho convertito in matrici X e Y perche non si usano in seguito)
```{r}
...
No polinomial regr.. la cond di linearity soddisfatta....

## Add categorical predictor

Till now we assumed that the response variable has equal mean/behaviour between the different states, a side by side is presented (using standardized and log transformed as in the previous model)
```{r}
boxplot(log(standf$ViolentCrimesPerPop+1)~standf$state, xlab="state", ylab=expression(log(ViolentCrimesPerPop+1)), col= c("#EF476F", "#FFD166", "#06D6A0", "#118AB2", "#073B4C"))
...
YF.train <- Y.train
YF.train[Y.train > 0] <- "High"
YF.train[!Y.train > 0]<- "Low"
YF.train <- as.factor(YF.train)

YF.test <- Y.test
YF.test[Y.test > 0] <- "High"
YF.test[!Y.test > 0]<- "Low"
...
roc.out <- roc(YF.train, logistic.prob, levels=c("Low", "High"))
...
logistic.pred[logistic.prob>0.667] <- "Low"

#train error
table(logistic.pred,YF.train)
1-mean(logistic.pred==YF.train)
...
train.sample <- as.numeric(sample(rownames(corrdf), 0.7*dim(corrdf)[1]))
nottrain <- setdiff(rownames(corrdf), train.sample)
valid.sample <- as.numeric(sample(nottrain, 0.15*dim(corrdf)[1]))
test.sample <- as.numeric(setdiff(nottrain, valid.sample))

X = subset(corrdf, select = - c(ViolentCrimesPerPop))
X.train <- subset(X, (row.names(X) %in% train.sample))
X.valid <- subset(X, (row.names(X) %in% valid.sample))
X.test <- subset(X, (row.names(X) %in% test.sample))

Y <- log(corrdf["ViolentCrimesPerPop",drop=FALSE] +1)

Y.train <- unlist(subset(Y, (row.names(Y) %in% train.sample)))
Y.valid <- unlist(subset(Y, (row.names(Y) %in% valid.sample)))
Y.test <- unlist(subset(Y, (row.names(Y) %in% test.sample)))
...
YF.valid <- Y.valid
YF.valid[Y.valid > 0] <- "High"
YF.valid[!Y.valid > 0]<- "Low"
YF.valid <- as.factor(YF.valid)
...
X.test <- as.matrix(X.test)
YF.train <- as.matrix(YF.train)
YF.valid <- as.matrix(YF.valid)
YF.test <- as.matrix(YF.test)
```
...
knnerr <- c()
for (i in 1:7) {
  knn.pred <- knn(X.train, X.valid, YF.train, k=i)
  #table(knn.pred,YF.train)
  knnerr[i] <- mean(knn.pred==YF.valid)
...
```{r}
# test error
knn.pred <- knn(X.train, X.test, YF.train, k=6)
